...
pgClient.query('SELECT * from location_weather', (err, res) => {
  ...
  const data_weather_bars = res.rows;
  console.log(data_weather_bars); // Verify the data

  // Store the data in a JavaScript file
  const fs = require('fs');
  fs.writeFileSync('data_weather_bars.js', `const data = ${JSON.stringify(data_weather_bars)};`);
